...
### Бізнес-правила:
Одному користувачу можуть належати 1+ профілів.
У профіля повинна бути вказана основна валюта.
Профіль може мати 0+ рахунків.
У переказі між рахунками беруть участь лише два рахунки *(Відправник та Одержувач)*.
До транзакції або регулярного платежу використовується категорія та підкатегорія.
***
...
4. **Accounts**
   ...
   - `account_name` - назва рахунку *(В нашому випадку це назва/тип картки або "Готівка")*.
...
5. **Transfers**
   ...
   - `payee_account_id` - ідентифікатор отримувача переказу.
   - `amount` - сума.
   - `fee` - комісія.
...
9. **Subcategories**
   ...
   Зв'язки:
   - Підкатегорія може належати лише одній категорії.
   - Декілька платежів можуть належати одній підкатегорії, можуть і не належати.
   - Декілька транзакцій можуть належати одній підкатегорії, можуть і не належати.
   - Лише одна підкатегорія може належати платежу або транзакції.
***
### Припущення чи обмеження:
- У користувача обов'язково повинні бути логін на пароль.
- Логін не повинен містити спецсимволи.
- Одному користувачу може належати не більше 3 профілів.
- Профіль може мати не більше 10 рахунків *(5х Дебетовий, 2х Кредитний, 3х Готівка)*.
- Припускається, що переказ може бути з одного свого рахунку на інший.
- `amount` та `balance` повинні бути **НЕ** цілими
- Дата наступного платежу не може бути у минулому, а транзакції та переказу у майбутньому.
